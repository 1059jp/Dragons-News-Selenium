...
def get_dragons_news():
    ...
    try:
        ...
        for item in items:
            ...
            # 「中日」か「ドラゴンズ」が入っていて、履歴にないものだけ採用
            if any(k in title for k in ['中日', 'ドラゴンズ']):
...
def create_html(news_list):
    ...
    <head>
        ...
        <style>
            ...
            .refresh-btn {{ margin-top:10px; padding:12px; border-radius:8px; border:none; background:white; color:#003399; font-weight:bold; cursor:pointer; width: 100%; }}
    ...
    <body>
        <div class="header">
            ...
            <button type="button" class="refresh-btn" onclick="window.location.reload(true);">🔄 情報を更新</button>
        </div>
    """
    for item in news_list:
        ...
        """
    if not news_list:
        html_content += "<p style='text-align:center; padding:50px; color:#666; background:white; border-radius:12px;'>新着ニュースはありません。</p>"
